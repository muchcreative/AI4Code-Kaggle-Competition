...
def create_quadrant_splits(code_count):
  quarter_loc, remainder = divmod(code_count, 4)
  quadrant_splits = np.arange(0, code_count-remainder+0.1, step=quarter_loc, dtype=np.int32)
...
def collect_md_code_groupings(c1_input_ids,
  ...
  for count, metadata in enumerate(tqdm(cell_metadata, desc='Collecting Markdown Code Groupings')):
    ...
    main_c1_input_ids = adjust_input_ids_len(c1_input_ids[doc_start+code_count:doc_start+doc_len], first_main_len)
    main_c2_input_ids = adjust_input_ids_len(c2_input_ids[doc_start+code_count:doc_start+doc_len], second_main_len)
    ...
    for md_idx in main_md_idxs:
      ...
      if code_count >= 4:
        ...
        code_rep1 = map_code_rep_to_input_ids(code_rep_idxs[0], sub_c1_input_ids, len(main_c1_input_id))
        code_rep2 = map_code_rep_to_input_ids(code_rep_idxs[1], sub_c2_input_ids, len(main_c2_input_id))
      else:
        code_rep_idxs = np.arange(code_count)
        code_rep1 = map_single_rep_to_input_ids(code_rep_idxs, sub_c1_input_ids, len(main_c1_input_id))
        code_rep2 = map_single_rep_to_input_ids(code_rep_idxs, sub_c2_input_ids, len(main_c2_input_id))
  ...
  return c1_groupings, c2_groupings, labels
...
def create_doc_code_rep(quarter_loc, quadrant_splits):
  if quarter_loc <= 3:
    ...
    for i in range(4):
      # Quad Rep 1
      quadrant_vector = np.arange(quadrant_splits[i], quadrant_splits[i+1])
      spacing = (len(quadrant_vector)-2) // 2
      quad_1_idxs = [0, spacing, spacing*2, -1]
      quad_rep_1 = quadrant_vector[quad_1_idxs]

      # Quad Rep 2
      if len(quadrant_vector) == 5:
        quad_2_idxs = [0, 1, 2, 3]
      elif len(quadrant_vector) == 6:
        quad_2_idxs = [1, 2, 3, 4]
      elif len(quadrant_vector) >= 7:
        reduced_vector = np.arange(quadrant_splits[i]+1, quadrant_splits[i+1]-1)
        spacing, remainder = divmod(len(reduced_vector)-2, 2)
        quad_2_idxs = [1, spacing+remainder, spacing*2+remainder, -2]
      else:
...
def map_single_rep_to_input_ids(code_rep, input_ids, main_len):
  avaliable_length = 512 - main_len - 6
...
def map_code_rep_to_input_ids(code_rep, input_ids, main_len):
  avaliable_length = 512 - main_len - 6
...
def create_features(groupings, tokenizer, disable_print=False):
  ...
  for md_input_id, grouping in tqdm(groupings, desc='Assembling Input Ids and Attention Masks', disable=disable_print):
    md_am = [1] * (len(md_input_id) + 2)  # For CLS and SEP token add 2
    markdown_input_id = [tokenizer.cls_token_id] + md_input_id + [tokenizer.sep_token_id]
...
def collect_best_code_groupings(c1_input_ids,
  ...
  for count, metadata in enumerate(cell_metadata):
    ...
    # Removes duplicates as the main markdown
    main_md_idxs, dupes = remove_duplicates(main_md_idxs, main_c1_input_ids)  # Only need one to reference
    ...
    doc_start += doc_len
  return c1_groupings, c2_groupings, dupes